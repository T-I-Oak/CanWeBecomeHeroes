...
import { TAGS, getTagBaseColors, getTagFramePaths, getTagPaths, getTagPrice, getTagWeight } from './TagCatalog.js';
...
export default class ItemFactory {
  ...
  createWeapon({ weapon, tags, x, y }) {
    const chip = new Chip({ id: 0, type: 'item', x, y, weight: Math.max(1, getTagWeight(tags)), centerPath: `/assets/items/hand-${weapon}.png`, tagPaths: getTagPaths(tags), tagFramePaths: getTagFramePaths(tags), tagBaseColors: getTagBaseColors(tags), bounds: GAME_AREAS.warehouse });
  ...
  createDestination({ destination, x, y }) {
    const tags = [];
    const chip = new Chip({ id: 0, type: 'item', x, y, weight: 1, centerPath: `/assets/items/hand-${destination}.png`, tagPaths: [], tagFramePaths: [], tagBaseColors: [], bounds: GAME_AREAS.warehouse, fillColor: AREA_THEME[DESTINATIONS[destination]].chipFill });
  ...
  createBodyItem({ part, tags = [], x, y, random = Math.random }) {
    ...
    const chip = new Chip({ id: 0, type: 'item', x, y, weight: Math.max(1, getTagWeight(tags)), centerPath: `/assets/items/${part}-${imageNumber}.png`, tagPaths: getTagPaths(tags), tagFramePaths: getTagFramePaths(tags), tagBaseColors: getTagBaseColors(tags), bounds: GAME_AREAS.warehouse });
