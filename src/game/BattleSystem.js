...
import { getTagBaseColors, getTagFramePaths, getTagGlyphScales, getTagPaths, getTagPrice, getTagWeight } from './TagCatalog.js';
...
export default class BattleSystem {
  ...
  applyOrb(actor, target, coefficient) {
    ...
    if (!item?.addTag('gem')) return; item.chip.weight = getTagWeight(item.tags); item.chip.tagPaths = getTagPaths(item.tags); item.chip.tagFramePaths = getTagFramePaths(item.tags); item.chip.tagBaseColors = getTagBaseColors(item.tags); item.chip.tagGlyphScales = getTagGlyphScales(item.tags); item.price = getTagPrice(item.tags); target.refreshDerivedValues?.();
  }
